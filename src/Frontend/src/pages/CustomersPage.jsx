...
const CustomersPage = () => {
    return (
        <div className="min-h-screen bg-gray-50" dir="rtl">
            {/* Added the overview_intro bar here */}
            <div className="overview_intro" style={{ position: 'fixed', background: '#F5F7E1', width: '100%', zIndex: '10', padding: '10px 50px', borderBottom: '1px solid #ddd' }}>
                <span className="overview" style={{ position: 'relative', bottom: '1px' }}>
                    <Link to="/" className="btn_go_home">الصفحة الرئيسية</Link> - عملاؤنا
                </span>
            </div>

            {/* Added pt-20 to ensure content starts after the fixed header */}
            <div className="pb-100 pt-204 container mx-auto px-60">
                ...
                <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 md:gap-8 lg:grid-cols-6">
                    {customers.map((item) => (
                        <div key={item.id} className="group flex flex-col items-center">
                            <div className="flex aspect-square w-full items-center justify-center rounded-2xl border border-gray-100 bg-white p-4 shadow-sm transition-all duration-500 group-hover:-translate-y-2 group-hover:shadow-xl">
                                <img
                                    src={"/images/" + item.image}
                                    alt={item.name}
                                    className="max-h-full max-w-full object-contain grayscale filter transition-all duration-500 group-hover:grayscale-0"
                                />
                            </div>
