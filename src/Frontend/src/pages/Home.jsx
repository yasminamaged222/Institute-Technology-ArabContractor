...
export default function Home() {
    ...
    return (
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        /* ── Feature cards ── */
        .feat-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:clamp(16px,2.5vw,28px);border:none;background:transparent;}
        ...
        .feat-card{
          ...
          border-radius:12px;
          background:${C.g1};
          box-shadow:0 4px 18px rgba(0,0,0,.07), 0 1px 4px rgba(0,0,0,.05);
        ...
        .feat-card:hover{
          background:${C.w};
          border-color:${C.o};
          box-shadow:0 8px 32px rgba(245,124,0,.18), 0 2px 8px rgba(0,0,0,.06);
        ...
        /* ── Downloads ── */
        .dl-row{display:flex;align-items:center;gap:16px;padding:20px 24px;border-bottom:1px solid rgba(255,255,255,.08);text-decoration:none;color:${C.w};transition:background .2s;border-radius:8px;}
        ...
        /* Container: featured card on right + 2×2 grid on left */
        .schools-layout{
          display:grid;
        ...
        /* Featured (Makawlen) card — tall rectangle */
        .school-featured{
          border-radius:12px;
        ...
        .school-featured:hover{
          transform:translateY(-6px);
          box-shadow:0 20px 48px rgba(8,101,168,.25);
        }

        /* 2×2 grid for the other 4 schools */
        .schools-other-grid{
          display:grid;
        ...
        @media(max-width:560px){
          .schools-other-grid{grid-template-columns:1fr;}
        }
        ...
        /* ── ob-outline link button ── */
        a.ob-outline{
          display:inline-flex;align-items:center;gap:8px;
          font-family:${F};font-size:clamp(.78rem,1.1vw,.88rem);font-weight:700;
          color:${C.o};text-decoration:none;
          border:1.5px solid ${C.o};
          padding:clamp(9px,1.2vw,12px) clamp(20px,2.8vw,32px);
          border-radius:8px;
        ...
        /* ══ LIBRARY — fully responsive 300px→2000px ══ */
        .lib-split{
          display:grid;
      ...
      `}</style>
            ...
            <section style={{ background: C.w }} ref={featRef}>
                ...
                <div className="W feat-grid" style={{ paddingTop: 0, paddingBottom: 'clamp(48px,6vw,80px)' }}>
                    {features.map((f, i) => (
                            ...
                            <div style={{ borderRight: '1px solid rgba(255,255,255,.15)', paddingRight: 'clamp(20px,3vw,40px)' }}>
                                {downloadItems.map((item, i) => (
                                    <a key={i} href={item.pdfUrl} target="_blank" rel="noopener noreferrer" className="dl-row">
                                        <div style={{ width: 44, height: 44, borderRadius: 8, background: 'rgba(255,255,255,.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                            ...
                            <div style={{ padding: '28px 24px 22px', borderBottom: `1px solid rgba(255,255,255,.15)` }}>
                                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8, marginBottom: 18 }}>
                                    <div style={{
                                        ...
                                        <SchoolIcon sx={{ fontSize: 32, color: C.w }} />
                                    </div>
                                    <span style={{
                                        fontFamily: F, fontSize: '.65rem', fontWeight: 700, letterSpacing: 1.2,
                                        padding: '4px 12px', borderRadius: 20,
                                        background: 'rgba(255,255,255,.15)',
                                        color: C.w,
                                        border: '1px solid rgba(255,255,255,.25)',
                                    }}>{makawlenSchool.tag}</span>
                                </div>
                                <h3 style={{
                                    fontFamily: F, fontSize: 'clamp(1rem,1.6vw,1.2rem)', fontWeight: 900,
                                    lineHeight: 1.45, color: C.w, margin: 0,
                                }}>{makawlenSchool.name}</h3>
                            </div>

                            {/* Body */}
                            <div style={{ padding: '20px 24px 28px', flex: 1, display: 'flex', flexDirection: 'column' }}>
                                <p style={{
                                    fontFamily: F, fontSize: 'clamp(.8rem,1.1vw,.92rem)',
                                    color: 'rgba(255,255,255,.75)', lineHeight: 1.9, marginBottom: 24, flex: 1,
                                }}>{makawlenSchool.desc}</p>
                                ...
                                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: 24 }}>
                                    {[
                                        ...
                                        <span key={idx} style={{
                                            fontFamily: F, fontSize: '.7rem', fontWeight: 700,
                                            color: 'rgba(255,255,255,.7)',
                                            display: 'flex', alignItems: 'center', gap: 5,
                                        }}>
                                            <Icon sx={{ fontSize: 14, color: 'rgba(255,255,255,.5)' }} />{text}
                                        </span>
                                    ))}
                                </div>
                                    ...
                                    <div style={{
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8, marginBottom: 12 }}>
                                            <div style={{
                                                ...
                                                background: 'rgba(8,101,168,.08)',
                                                display: 'flex', alignItems: 'center', justifyContent: 'center',
                                            }}>
                                                <sc.IconComp sx={{ fontSize: 24, color: C.b }} />
                                            </div>
                                            <span style={{
                                                fontFamily: F, fontSize: '.6rem', fontWeight: 700, letterSpacing: 1.2,
                                                padding: '3px 10px', borderRadius: 20,
                                                background: 'rgba(8,101,168,.1)',
                                                color: C.b,
                                                border: '1px solid rgba(8,101,168,.2)',
                                            }}>{sc.tag}</span>
                                        </div>
                                        <h3 style={{ fontFamily: F, fontSize: 'clamp(.8rem,1.1vw,.9rem)', fontWeight: 800, lineHeight: 1.45, color: C.k, margin: 0 }}>{sc.name}</h3>
                                    </div>

                                    {/* Body */}
                                    <div style={{ padding: '14px 18px 18px', flex: 1, display: 'flex', flexDirection: 'column' }}>
                                        <p style={{ fontFamily: F, fontSize: 'clamp(.72rem,.95vw,.82rem)', color: C.g5, lineHeight: 1.8, marginBottom: 14, flex: 1 }}>{sc.desc}</p>
                                        ...
                                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 16 }}>
                                            ...
                                            <span className="sc-meta"><GroupsIcon sx={{ fontSize: 13, color: C.b }} />{sc.students !== '—' ? `${sc.students} طالب` : 'متعدد'}</span>
                                        </div>

                                        <Link to="/technical-education" style={{
                                            display: 'inline-flex', alignItems: 'center', gap: 6,
                                            fontFamily: F, fontSize: '.74rem', fontWeight: 700,
                    ...
                    <div className="lib-split">
                        ...
                        <div className="lib-visual">
                            <div style={{
                                ...
                                border: '1px solid rgba(255,255,255,.12)', top: -110, right: -110, pointerEvents: 'none',
                            }} />
                            <div style={{ position: 'relative', zIndex: 1, textAlign: 'center', width: '100%' }}>
                                <LibraryBooksIcon sx={{ fontSize: 'clamp(36px,5.5vw,64px)', color: C.w, marginBottom: 10 }} />
                                <div style={{
                                    fontFamily: F,
                                    fontSize: 'clamp(.95rem,2vw,1.7rem)',
                                    fontWeight: 900, color: C.w,
                                    lineHeight: 1.2, marginBottom: 12,
                                }}>المكتبة العلمية المتخصصة</div>
                                <div style={{ width: 32, height: 2, background: 'rgba(255,255,255,.5)', margin: '0 auto 14px' }} />
                                <div className="lib-tags">
                                    {['5,000+ كتاب', '200+ دورية', 'رقمية'].map((t, i) => (
                                        <span key={i} className="lib-tag">
                                            <AutoStoriesIcon sx={{ fontSize: 11 }} /> {t}
                                        </span>
                                    ))}
                                </div>
                            </div>
