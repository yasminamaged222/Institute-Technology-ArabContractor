...
const Instructors = () => {
    ...
    useEffect(() => {
        const fetchLecturers = async () => {
            try {
                ...
                if (!response.ok) {
                    throw new Error('فشل في تحميل بيانات المحاضرين');
                }
    ...
    // Parse HTML string to extract text content
    const parseHTMLContent = (htmlString) => {
        if (!htmlString) return [];

        const div = document.createElement('div');
        div.innerHTML = htmlString;
        const paragraphs = div.querySelectorAll('p, h5');
    ...
    const CVDialog = ({ lecturer, onClose }) => {
        ...
        const educationList = parseHTMLContent(lecturer.edu);
        const detailsList = parseHTMLContent(lecturer.details);
