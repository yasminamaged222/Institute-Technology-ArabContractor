...
const ADMIN_EMAILS = ['yasminamaged22@gmail.com', 'abeer.naguib@gmail.com', 'amrshamy91@gmail.com', 'abdelmawla1642@gmail.com', 'mostafa.awaad@gmail.com', 'samiryousri96@gmail.com','mahmoud_salah@arabcont.com']; // ← عدّلها
...
const Navbar = () => {
    ...
    const aboutLinks = ['نبذة عامة', 'الرؤية والأهداف', 'الشهادات والاعتمادات', 'فريق العمل', 'قائمة المحاضرين', 'التدريب الأونلاين','برنامج اعداد مهندس مكتب فني ','الخطة التدريبية', 'التقرير الشهرى', 'مكتبة الصور والفيديوهات', 'البروتوكولات والإتفاقيات', 'عملاؤنا'];
    const aboutLinkPaths = { 'نبذة عامة': '/overview', 'الرؤية والأهداف': '/mission', 'الشهادات والاعتمادات': '/certifications', 'فريق العمل': '/team', 'قائمة المحاضرين': '/instructors', 'الخطة التدريبية': '/pdf/ICEMT_Plan_Training.pdf', 'التقرير الشهرى': '/pdf/ICEMT_Monthly_Activity.pdf', 'مكتبة الصور والفيديوهات': '/gallery', 'البروتوكولات والإتفاقيات': '/protocols', 'عملاؤنا': '/customers', 'التدريب الأونلاين': '/online-training', 'برنامج اعداد مهندس مكتب فني ':'/cea-program' };
                ...
                <Toolbar sx={{ justifyContent: 'space-between', display: 'flex', px: { xs: 1, md: 4 } }}>
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexShrink: 0 }}>
                        ...
                        <Stack direction="row" alignItems="center" component={Link} to="/" sx={{ textDecoration: 'none', color: 'inherit', flexShrink: 0 }}>
                            ...
                            {!isSmallMobile && (<Box sx={{ mr: 1, textAlign: 'left' }}>
                                <Typography variant="body2" sx={{ fontWeight: 'bold', color: '#0865a8', fontSize: '0.8rem', whiteSpace: 'nowrap', fontFamily: '"Noto Kufi Arabic",serif' }}>المقاولون العرب</Typography>
                                <Typography variant="caption" sx={{ color: '#000', display: { xs: 'none', md: 'block' }, fontFamily: '"Noto Kufi Arabic",serif' }}>المعهد التكنولوجى لهندسة التشييد والإدارة</Typography>
                            </Box>)}
                        </Stack>
                    </Box>
                    ...
                    {!isMobile && (
                        <div className="courses-dropdown-wrapper">
                            <Button color="inherit" endIcon={<KeyboardArrowDownIcon />} sx={{ px: 2, mx: 1, whiteSpace: 'nowrap', fontFamily: '"Noto Kufi Arabic",serif', color: '#000', '&:hover': { bgcolor: 'rgba(8,101,168,0.08)', color: '#0865a8' } }}>الدورات التدريبية</Button>
                            <div className="courses-dropdown-menu">
                                <Box sx={{ display: 'flex', height: '420px' }} dir="rtl">
                                    <Box sx={{ width: '280px', borderLeft: '1px solid #e0e0e0', bgcolor: '#fafafa', overflowY: 'auto', '&::-webkit-scrollbar': { width: '6px' }, '&::-webkit-scrollbar-thumb': { backgroundColor: '#d0d0d0', borderRadius: '3px' } }}>
                                        <List sx={{ p: 1 }}>
                                            {loading ? (<ListItemButton><ListItemText primary="جاري التحميل..." primaryTypographyProps={{ fontSize: 13, fontFamily: '"Noto Kufi Arabic",serif', color: '#666' }} /></ListItemButton>)
                                                : error ? (<ListItemButton><ListItemText primary="حدث خطأ في التحميل" primaryTypographyProps={{ fontSize: 13, fontFamily: '"Noto Kufi Arabic",serif', color: '#f44336' }} /></ListItemButton>)
                                                    : mainCourses.map(cat => (
                                    ...
                                    <Box sx={{ flex: 1, p: 2.5, bgcolor: 'white', overflowY: 'auto', '&::-webkit-scrollbar': { width: '6px' }, '&::-webkit-scrollbar-thumb': { backgroundColor: '#d0d0d0', borderRadius: '3px' } }}>
                                        {loading ? (<Box sx={{ textAlign: 'center', py: 4 }}><Typography sx={{ color: '#666', fontFamily: '"Noto Kufi Arabic",serif' }}>جاري التحميل...</Typography></Box>)
                                            : error ? (<Box sx={{ textAlign: 'center', py: 4 }}><Typography sx={{ color: '#f44336', fontFamily: '"Noto Kufi Arabic",serif' }}>حدث خطأ في التحميل</Typography></Box>)
                                                : activeCategory ? (<>
                                                    ...
                                                    {activeCategory.link && !activeCategory.sub && (
                                                        <Box sx={{ textAlign: 'center', py: 4, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
                                                            <Typography sx={{ color: '#666', fontFamily: '"Noto Kufi Arabic",serif', fontSize: '0.9rem', mb: 1 }}>انقر على الفئة للانتقال إلى الصفحة</Typography>
                                                            <Button component={Link} to={activeCategory.link} variant="contained" sx={{ bgcolor: '#0865a8', color: 'white', fontFamily: '"Noto Kufi Arabic",serif', px: 4, py: 1, borderRadius: 2, textTransform: 'none', '&:hover': { bgcolor: '#f57c00' } }}>عرض التفاصيل</Button>
                                                        </Box>
                                                    )}
                                ...
                                <Paper elevation={6} sx={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 1300, borderRadius: '0 0 12px 12px', border: '1px solid #0865a8', borderTop: 'none', boxShadow: '0 8px 32px rgba(0,0,0,0.14)', overflow: 'hidden', bgcolor: 'white', maxHeight: '420px', overflowY: 'auto', '&::-webkit-scrollbar': { width: '5px' }, '&::-webkit-scrollbar-thumb': { bgcolor: '#d0d0d0', borderRadius: '3px' } }}>
                                    {suggestions.length === 0 && !searchValue.trim() && recentSearches.length > 0 && (<>
                                        <Box sx={{ px: 2, pt: 1.5, pb: 0.5, display: 'flex', alignItems: 'center', gap: 0.5 }}>
                                            ...
                                            <Typography sx={{ fontSize: '0.7rem', color: '#aaa', fontFamily: '"Noto Kufi Arabic",serif', fontWeight: 'bold', letterSpacing: 0.6 }}>عمليات البحث الأخيرة</Typography>
                                        </Box>
                                        {recentSearches.map((term, i) => (
                                            <Box key={term} onMouseDown={() => { saveRecentSearch(term); setSearchValue(term); setSearchFocused(false); const matched = allCourses.filter(c => c.title.toLowerCase().includes(term.toLowerCase()) || c.description.toLowerCase().includes(term.toLowerCase())); navigate(`/search?q=${encodeURIComponent(term)}`, { state: { results: matched, query: term } }); }} onMouseEnter={() => setActiveSuggestionIndex(i)} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', px: 2, py: 0.9, cursor: 'pointer', bgcolor: activeSuggestionIndex === i ? 'rgba(8,101,168,0.06)' : 'transparent', '&:hover': { bgcolor: 'rgba(8,101,168,0.06)' }, transition: 'background 0.15s' }}>
                                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                                                    ...
                                                    <Typography sx={{ fontFamily: '"Noto Kufi Arabic",serif', fontSize: '0.87rem', color: '#333', direction: 'rtl' }}>{term}</Typography>
                                                </Box>
                                    ...
                                    {coursesLoading && !coursesLoaded && searchValue.trim() && (<Box sx={{ px: 2, py: 2, textAlign: 'center' }}><Typography sx={{ color: '#aaa', fontFamily: '"Noto Kufi Arabic",serif', fontSize: '0.82rem' }}>جاري تحميل الدورات...</Typography></Box>)}
                                    {suggestions.length > 0 && (<>
                                        <Box sx={{ px: 2, pt: 1.5, pb: 0.8, display: 'flex', alignItems: 'center', gap: 0.8, borderBottom: '1px solid #f5f5f5' }}>
                                            ...
                                            <Typography sx={{ fontSize: '0.7rem', color: '#999', fontFamily: '"Noto Kufi Arabic",serif', fontWeight: 'bold', letterSpacing: 0.5 }}>الدورات المطابقة ({suggestions.length})</Typography>
                                        </Box>
                                                ...
                                                <Box key={course.id} onMouseDown={e => { e.preventDefault(); saveRecentSearch(title); setSearchValue(''); setSuggestions([]); setSearchFocused(false); setActiveSuggestionIndex(-1); navigate(`/course/${course.slug}`); }} onMouseEnter={() => setActiveSuggestionIndex(i)} sx={{ display: 'flex', alignItems: 'center', gap: 1.5, px: 1.5, py: 1, cursor: 'pointer', bgcolor: activeSuggestionIndex === i ? 'rgba(8,101,168,0.05)' : 'transparent', '&:hover': { bgcolor: 'rgba(8,101,168,0.05)' }, transition: 'background 0.15s', borderBottom: i < suggestions.length - 1 ? '1px solid #fafafa' : 'none' }}>
                                                    ...
                                                    <Box sx={{ flexGrow: 1, minWidth: 0, direction: 'rtl' }}>
                                                        ...
                                                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.6, mt: 0.4 }}>
                                                            <Box sx={{ display: 'inline-flex', alignItems: 'center', bgcolor: 'rgba(8,101,168,0.09)', px: 0.8, py: 0.2, borderRadius: '4px', flexShrink: 0 }}><Typography sx={{ fontFamily: '"Noto Kufi Arabic",serif', fontSize: '0.68rem', color: '#0865a8', fontWeight: 'bold' }}>دورة</Typography></Box>
                                    ...
                                    {!coursesLoading && coursesLoaded && searchValue.trim() && suggestions.length === 0 && (<Box sx={{ px: 2, py: 2, textAlign: 'center' }}><Typography sx={{ color: '#aaa', fontFamily: '"Noto Kufi Arabic",serif', fontSize: '0.82rem' }}>لا توجد نتائج لـ &quot;{searchValue}&quot;</Typography></Box>)}
                                    {searchValue.trim() && (<Box onMouseDown={e => { e.preventDefault(); executeSearch(searchValue); }} sx={{ px: 2, py: 1, borderTop: '1px solid #f0f0f0', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 1, bgcolor: 'rgba(8,101,168,0.02)', '&:hover': { bgcolor: 'rgba(8,101,168,0.08)' }, transition: 'background 0.15s' }}><SearchIcon sx={{ fontSize: 15, color: '#0865a8' }} /><Typography sx={{ fontFamily: '"Noto Kufi Arabic",serif', fontSize: '0.82rem', color: '#0865a8', fontWeight: 'bold', direction: 'rtl' }}>عرض جميع النتائج لـ &quot;{searchValue}&quot;</Typography></Box>)}
                                </Paper>
                            )}
                        </Box>
                    </ClickAwayListener>
                        ...
                        {!isMobile && (
                            <Stack direction="row" spacing={0.5} alignItems="center">
                                <div className="about-dropdown-wrapper">
                                    <Button color="inherit" endIcon={<KeyboardArrowDownIcon />} sx={{ px: 1.5, fontFamily: '"Noto Kufi Arabic",serif', color: '#000', fontSize: '0.9rem', '&:hover': { bgcolor: 'rgba(8,101,168,0.08)', color: '#0865a8' } }}>عن المعهد</Button>
                                ...
                                <Button color="inherit" component={Link} to="/news" sx={{ px: 1.5, fontFamily: '"Noto Kufi Arabic",serif', color: '#000', fontSize: '0.9rem', '&:hover': { bgcolor: 'rgba(8,101,168,0.08)', color: '#0865a8' } }}>الأخبار</Button>
                                <Button color="inherit" component={Link} to="/library" sx={{ px: 1.5, fontFamily: '"Noto Kufi Arabic",serif', color: '#000', fontSize: '0.9rem', '&:hover': { bgcolor: 'rgba(8,101,168,0.08)', color: '#0865a8' } }}>المكتبة</Button>
                                <div className="services-dropdown-wrapper">
                                    <Button color="inherit" endIcon={<KeyboardArrowDownIcon />} sx={{ px: 1.5, fontFamily: '"Noto Kufi Arabic",serif', color: '#000', fontSize: '0.9rem', '&:hover': { bgcolor: 'rgba(8,101,168,0.08)', color: '#0865a8' } }}>الخدمات</Button>
            ...
            <Drawer anchor="left" open={mobileOpen} onClose={toggleDrawer(false)}>
                <Box sx={{ width: 300, p: 2, bgcolor: 'white', height: '100%' }} dir="rtl">
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3, pb: 2, borderBottom: '2px solid #0865a8' }}>
                        <Typography variant="h6" sx={{ fontFamily: '"Noto Kufi Arabic",serif', color: '#0865a8', fontWeight: 'bold' }}>القائمة</Typography>
                    ...
                    <List>
                        <ListItemButton onClick={() => setMobileCoursesOpen(!mobileCoursesOpen)} sx={{ bgcolor: mobileCoursesOpen ? 'rgba(8,101,168,0.08)' : 'transparent', borderRadius: 1, mb: 0.5, '&:hover': { bgcolor: 'rgba(8,101,168,0.12)' } }}>
                            <ListItemText primary="الدورات التدريبية" sx={{ textAlign: 'left', '& .MuiTypography-root': { fontFamily: '"Noto Kufi Arabic",serif', fontWeight: 'bold', color: '#0865a8' } }} />
                        ...
                        <Collapse in={mobileCoursesOpen} timeout="auto" unmountOnExit>
                            <List component="div" disablePadding sx={{ pr: 2 }}>
                                {loading ? (<ListItemButton><ListItemText primary="جاري التحميل..." sx={{ '& .MuiTypography-root': { fontFamily: '"Noto Kufi Arabic",serif', fontSize: '0.9rem', color: '#666' } }} /></ListItemButton>)
                                    : error ? (<ListItemButton><ListItemText primary="حدث خطأ في التحميل" sx={{ '& .MuiTypography-root': { fontFamily: '"Noto Kufi Arabic",serif', fontSize: '0.9rem', color: '#f44336' } }} /></ListItemButton>)
                        ...
                        <SignedIn>
                            <ListItemButton component={Link} to="/my-courses" onClick={toggleDrawer(false)} sx={{ borderRadius: 1, mb: 0.5, bgcolor: 'rgba(8,101,168,0.05)', '&:hover': { bgcolor: 'rgba(8,101,168,0.12)' } }}>
                                ...
                                <ListItemText primary="دوراتي" primaryTypographyProps={{ fontFamily: '"Noto Kufi Arabic",serif', fontWeight: 'bold', color: '#0865a8' }} />
                            </ListItemButton>
                            <Divider sx={{ my: 2, bgcolor: '#0865a8', height: 2 }} />
                        </SignedIn>
                        ...
                        <ListItemButton onClick={() => setMobileAboutOpen(!mobileAboutOpen)} sx={{ bgcolor: mobileAboutOpen ? 'rgba(8,101,168,0.08)' : 'transparent', borderRadius: 1, mb: 0.5, '&:hover': { bgcolor: 'rgba(8,101,168,0.12)' } }}>
                            <ListItemText primary="عن المعهد" sx={{ textAlign: 'left', '& .MuiTypography-root': { fontFamily: '"Noto Kufi Arabic",serif', fontWeight: 'bold', color: '#0865a8' } }} />
                        ...
                        <ListItemButton onClick={() => setMobileServicesOpen(!mobileServicesOpen)} sx={{ bgcolor: mobileServicesOpen ? 'rgba(8,101,168,0.08)' : 'transparent', borderRadius: 1, mb: 0.5, '&:hover': { bgcolor: 'rgba(8,101,168,0.12)' } }}>
                            <ListItemText primary="الخدمات" sx={{ textAlign: 'left', '& .MuiTypography-root': { fontFamily: '"Noto Kufi Arabic",serif', fontWeight: 'bold', color: '#0865a8' } }} />
                            {mobileServicesOpen ? <ExpandMoreIcon sx={{ color: '#0865a8' }} /> : <ChevronLeftIcon sx={{ color: '#0865a8' }} />}
                        </ListItemButton>
                        <Collapse in={mobileServicesOpen} timeout="auto" unmountOnExit>
                            <List component="div" disablePadding sx={{ pr: 2 }}>
                                {serviceLinks.map((item, idx) => (<ListItemButton key={idx} component={Link} to={item.path} onClick={toggleDrawer(false)} sx={{ '&:hover': { bgcolor: 'rgba(245,124,0,0.08)' } }}><ListItemText primary={item.title} primaryTypographyProps={{ fontFamily: '"Noto Kufi Arabic",serif', fontSize: '0.85rem' }} /></ListItemButton>))}
                            </List>
                        </Collapse>
                        <Divider sx={{ my: 2, bgcolor: '#0865a8', height: 2 }} />
                        <ListItemButton component={Link} to="/news" onClick={toggleDrawer(false)} sx={{ borderRadius: 1, mb: 0.5, '&:hover': { bgcolor: 'rgba(245,124,0,0.08)' } }}><ListItemText primary="الأخبار" primaryTypographyProps={{ fontFamily: '"Noto Kufi Arabic",serif', fontWeight: 'bold', color: '#0865a8' }} /></ListItemButton>
                        <ListItemButton component={Link} to="/library" onClick={toggleDrawer(false)} sx={{ borderRadius: 1, '&:hover': { bgcolor: 'rgba(245,124,0,0.08)' } }}><ListItemText primary="المكتبة" primaryTypographyProps={{ fontFamily: '"Noto Kufi Arabic",serif', fontWeight: 'bold', color: '#0865a8' }} /></ListItemButton>
                    </List>
                </Box>
            </Drawer>
