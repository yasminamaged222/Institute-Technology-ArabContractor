...
const Navbar = () => {
    ...
    // Helper: pick the right label based on current language
    ...
    useEffect(() => {
        ...
        suggestionDebounce.current = setTimeout(() => {
            ...
            setSuggestions(allCourses.filter(c => c.title.toLowerCase().includes(q) || c.description.toLowerCase().includes(q) || c.category.toLowerCase().includes(q) || c.place.toLowerCase().includes(q)).slice(0, 10));
        }, 200);
    ...
    return (
        <>
            <style>{`
                ...
                .dropdown-menu-item{padding:10px 16px;cursor:pointer;transition:all 0.2s;text-decoration:none;color:#000;display:block;font-family:"Noto Kufi Arabic",serif;font-size:0.85rem;}
                ...
                .admin-nav-btn{display:inline-flex!important;align-items:center!important;justify-content:center!important;gap:3px!important;width:72px!important;height:26px!important;padding:0!important;background:#f57c00!important;color:#fff!important;border-radius:6px!important;font-size:0.7rem!important;font-weight:400!important;text-decoration:none!important;border:2px solid #f57c00!important;transition:all 0.2s!important;cursor:pointer!important;white-space:nowrap!important;flex-shrink:0!important;line-height:1!important;}
                .admin-nav-btn:hover{background:#fff!important;color:#f57c00!important;box-shadow:0 3px 10px rgba(245,124,0,0.28)!important;}
            `}</style>

            <AppBar position="fixed" elevation={4} sx={{ bgcolor: 'white', color: '#000', py: 0.5, top: 0, zIndex: 1100, height: 70 }}>
                <Toolbar sx={{ justifyContent: 'space-between', display: 'flex', px: { xs: 1, md: 4 } }}>
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexShrink: 0 }}>
                        ...
                        <Stack direction="row" alignItems="center" component={Link} to="/" sx={{ textDecoration: 'none', color: 'inherit', flexShrink: 0 }}>
                            ...
                            {!isSmallMobile && (
                                <Box sx={{ mx: 1, textAlign: isRTL ? 'right' : 'left' }}>
                                    <Typography variant="body2" sx={{ fontWeight: 'bold', color: '#0865a8', fontSize: '0.8rem', whiteSpace: 'nowrap', fontFamily: '"Noto Kufi Arabic",serif' }}>
                                        {isRTL ? 'المقاولون العرب' : 'Arab Contractors'}
                                    </Typography>
                                    <Typography variant="caption" sx={{ color: '#000', display: { xs: 'none', md: 'block' }, fontFamily: '"Noto Kufi Arabic",serif' }}>
                    ...
                    {!isMobile && (
                        <div className="courses-dropdown-wrapper">
                            <Button color="inherit" endIcon={<KeyboardArrowDownIcon />} sx={{ px: 2, mx: 1, whiteSpace: 'nowrap', fontFamily: '"Noto Kufi Arabic",serif', color: '#000', '&:hover': { bgcolor: 'rgba(8,101,168,0.08)', color: '#0865a8' } }}>
                                {t('nav.courses')}
                            </Button>
                            <div className="courses-dropdown-menu">
                                <Box sx={{ display: 'flex', height: '420px' }} dir={isRTL ? 'rtl' : 'ltr'}>
                                    <Box sx={{ width: '280px', borderLeft: '1px solid #e0e0e0', bgcolor: '#fafafa', overflowY: 'auto', '&::-webkit-scrollbar': { width: '6px' }, '&::-webkit-scrollbar-thumb': { backgroundColor: '#d0d0d0', borderRadius: '3px' } }}>
                                        <List sx={{ p: 1 }}>
                                            {loading ? (<ListItemButton><ListItemText primary={t('common.loading')} primaryTypographyProps={{ fontSize: 13, fontFamily: '"Noto Kufi Arabic",serif', color: '#666' }} /></ListItemButton>)
                                                : error ? (<ListItemButton><ListItemText primary={t('common.error')} primaryTypographyProps={{ fontSize: 13, fontFamily: '"Noto Kufi Arabic",serif', color: '#f44336' }} /></ListItemButton>)
                                                    : mainCourses.map(cat => (
                                    ...
                                    <Box sx={{ flex: 1, p: 2.5, bgcolor: 'white', overflowY: 'auto', '&::-webkit-scrollbar': { width: '6px' }, '&::-webkit-scrollbar-thumb': { backgroundColor: '#d0d0d0', borderRadius: '3px' } }}>
                                        {loading ? (<Box sx={{ textAlign: 'center', py: 4 }}><Typography sx={{ color: '#666', fontFamily: '"Noto Kufi Arabic",serif' }}>{t('common.loading')}</Typography></Box>)
                                            : error ? (<Box sx={{ textAlign: 'center', py: 4 }}><Typography sx={{ color: '#f44336', fontFamily: '"Noto Kufi Arabic",serif' }}>{t('common.error')}</Typography></Box>)
                                                : activeCategory ? (<>
                                                    ...
                                                    {activeCategory.link && !activeCategory.sub && (
                                                        <Box sx={{ textAlign: 'center', py: 4, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
                                                            <Typography sx={{ color: '#666', fontFamily: '"Noto Kufi Arabic",serif', fontSize: '0.9rem', mb: 1 }}>
                                                                {t('courses.clickToView')}
                                                            </Typography>
                                                            <Button component={Link} to={activeCategory.link} variant="contained" sx={{ bgcolor: '#0865a8', color: 'white', fontFamily: '"Noto Kufi Arabic",serif', px: 4, py: 1, borderRadius: 2, textTransform: 'none', '&:hover': { bgcolor: '#f57c00' } }}>
                                                                {t('courses.details')}
                                                            </Button>
                                                        </Box>
                                                    )}
                                                    {activeCategory.sub && (<Box>{activeCategory.sub.map((sub, idx) => (
                                                        <Box key={sub.id} sx={{ mb: 2.5, pb: 2, borderBottom: idx < activeCategory.sub.length - 1 ? '1px solid #f0f0f0' : 'none' }}>
                                                            <Box sx={{ display: 'flex', alignItems: 'center', mb: 1.5, gap: 1 }}>
                                                                <Box sx={{ width: 24, height: 24, borderRadius: '50%', bgcolor: '#f0f7ff', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><Typography sx={{ fontSize: 11, fontWeight: 'bold', color: '#0865a8', fontFamily: '"Noto Kufi Arabic",serif' }}>{idx + 1}</Typography></Box>
                                                            ...
                                                            {sub.topics && (<Box sx={{ pr: 4 }}>{sub.topics.map(topic => (
                                                                <Link key={topic.id} to={topic.link} style={{ textDecoration: 'none' }}>
                                                                    <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 1, py: 0.8, px: 1.5, mb: 0.5, borderRadius: 1, cursor: 'pointer', transition: 'all 0.2s', '&:hover': { bgcolor: 'rgba(245,124,0,0.08)', transform: isRTL ? 'translateX(-4px)' : 'translateX(4px)', '& .topic-bullet': { bgcolor: '#f57c00' } } }}>
                                                                        ...
                                                                        <Typography sx={{ fontSize: '0.8rem', fontFamily: '"Noto Kufi Arabic",serif', color: '#444', lineHeight: 1.6 }}>{topic.name.length > 50 ? topic.name.substring(0, 50) + '...' : topic.name}</Typography>
                                                                    </Box>
                                                                </Link>
                                                            ))}</Box>)}
                                                        </Box>
                        ...
                        <Box ref={searchBoxRef} sx={{ display: 'flex', alignItems: 'center', flexGrow: 0, flexShrink: 1, justifyContent: 'center', width: { xs: 'auto', sm: '350px', md: '450px', lg: '550px' }, maxWidth: { xs: 'calc(100% - 200px)', sm: '350px', md: '450px', lg: '550px' }, mx: { xs: 0.5, sm: 1, md: 2 }, position: 'relative', minWidth: { xs: '120px', sm: '250px' } }}>
                            <Box component="form" onSubmit={handleSearchSubmit} role="search" sx={{ bgcolor: searchFocused ? '#ffffff' : '#f1f3f4', borderRadius: searchFocused && dropdownVisible ? '24px 24px 0 0' : 9, display: 'flex', alignItems: 'center', px: 2, py: 0.8, width: '100%', transition: 'all 0.2s', border: searchFocused ? '1px solid #0865a8' : '1px solid transparent', boxShadow: searchFocused ? '0 0 0 2px rgba(8,101,168,0.1)' : 'none', borderBottom: searchFocused && dropdownVisible ? '1px solid #e0e0e0' : undefined }}>
                                <InputBase
                                    ...
                                    sx={{ color: '#000', flexGrow: 1, textAlign: isRTL ? 'right' : 'left', fontFamily: '"Noto Kufi Arabic",serif', fontSize: { xs: '0.8rem', md: '0.95rem' }, pr: 1 }}
                                />
                                {searchValue && (<IconButton size="small" onClick={() => { setSearchValue(''); setSuggestions([]); setActiveSuggestionIndex(-1); searchInputRef.current?.focus(); }} sx={{ p: 0.5, mr: 0.5 }} aria-label={t('common.cancel')}><CloseIcon sx={{ fontSize: 16, color: '#888' }} /></IconButton>)}
                                <IconButton type="submit" disabled={!searchValue.trim()} sx={{ p: 0.5, '&:hover': { bgcolor: 'transparent' }, '&.Mui-disabled': { opacity: 0.4 } }} aria-label={t('courses.search')}><SearchIcon sx={{ color: '#0865a8', fontSize: { xs: 18, md: 22 } }} /></IconButton>
                            </Box>
                                        ...
                                        {recentSearches.map((term, i) => (
                                            <Box key={term} onMouseDown={() => { saveRecentSearch(term); setSearchValue(term); setSearchFocused(false); const matched = allCourses.filter(c => c.title.toLowerCase().includes(term.toLowerCase()) || c.description.toLowerCase().includes(term.toLowerCase())); navigate(`/search?q=${encodeURIComponent(term)}`, { state: { results: matched, query: term } }); }} onMouseEnter={() => setActiveSuggestionIndex(i)} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', px: 2, py: 0.9, cursor: 'pointer', bgcolor: activeSuggestionIndex === i ? 'rgba(8,101,168,0.06)' : 'transparent', '&:hover': { bgcolor: 'rgba(8,101,168,0.06)' }, transition: 'background 0.15s' }}>
                                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                                                    ...
                                                    <Typography sx={{ fontFamily: '"Noto Kufi Arabic",serif', fontSize: '0.87rem', color: '#333', direction: isRTL ? 'rtl' : 'ltr' }}>{term}</Typography>
                                                </Box>
                                                <IconButton size="small" onMouseDown={e => removeRecentSearch(term, e)} sx={{ p: 0.3, color: '#ccc', '&:hover': { color: '#f57c00' } }} aria-label={t('common.delete')}><CloseIcon sx={{ fontSize: 12 }} /></IconButton>
                                            </Box>
                                        ))}
                                        <Box sx={{ height: '1px', bgcolor: '#f5f5f5', mx: 2 }} />
                                    </>)}
                                    {coursesLoading && !coursesLoaded && searchValue.trim() && (<Box sx={{ px: 2, py: 2, textAlign: 'center' }}><Typography sx={{ color: '#aaa', fontFamily: '"Noto Kufi Arabic",serif', fontSize: '0.82rem' }}>{t('search.loadingCourses')}</Typography></Box>)}
                                            ...
                                            return (
                                                <Box key={course.id} onMouseDown={e => { e.preventDefault(); saveRecentSearch(title); setSearchValue(''); setSuggestions([]); setSearchFocused(false); setActiveSuggestionIndex(-1); navigate(`/course/${course.slug}`); }} onMouseEnter={() => setActiveSuggestionIndex(i)} sx={{ display: 'flex', alignItems: 'center', gap: 1.5, px: 1.5, py: 1, cursor: 'pointer', bgcolor: activeSuggestionIndex === i ? 'rgba(8,101,168,0.05)' : 'transparent', '&:hover': { bgcolor: 'rgba(8,101,168,0.05)' }, transition: 'background 0.15s', borderBottom: i < suggestions.length - 1 ? '1px solid #fafafa' : 'none' }}>
                                                    <Box sx={{ width: 52, height: 52, flexShrink: 0, borderRadius: '10px', overflow: 'hidden', background: 'linear-gradient(135deg,#0865a8 0%,#f57c00 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 3px 8px rgba(8,101,168,0.25)', position: 'relative' }}>
                                                        {course.image ? (<Box component="img" src={course.image} alt={title} sx={{ width: '100%', height: '100%', objectFit: 'cover' }} />) : (<Box sx={{ bgcolor: 'rgba(255,255,255,0.15)', borderRadius: '50%', p: '7px', backdropFilter: 'blur(4px)', border: '1.5px solid rgba(255,255,255,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><svg width="20" height="20" fill="none" stroke="white" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" /></svg></Box>)}
                                                    </Box>
                                                    <Box sx={{ flexGrow: 1, minWidth: 0, direction: isRTL ? 'rtl' : 'ltr' }}>
                                                        ...
                                                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.6, mt: 0.4 }}>
                                                            <Box sx={{ display: 'inline-flex', alignItems: 'center', bgcolor: 'rgba(8,101,168,0.09)', px: 0.8, py: 0.2, borderRadius: '4px', flexShrink: 0 }}><Typography sx={{ fontFamily: '"Noto Kufi Arabic",serif', fontSize: '0.68rem', color: '#0865a8', fontWeight: 'bold' }}>{t('search.courseTag')}</Typography></Box>
                                                            {(course.category || course.place) && (<Typography sx={{ fontFamily: '"Noto Kufi Arabic",serif', fontSize: '0.72rem', color: '#888', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{course.category || course.place}</Typography>)}
                                                        </Box>
                                                    </Box>
                                                    <Box sx={{ opacity: activeSuggestionIndex === i ? 1 : 0, transition: 'opacity 0.15s', color: '#0865a8', flexShrink: 0 }}><svg width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d={isRTL ? "M15 19l-7-7 7-7" : "M9 5l7 7-7 7"} /></svg></Box>
                                                </Box>
                                            );
                                        })}
                                    </>)}
                                    {!coursesLoading && coursesLoaded && searchValue.trim() && suggestions.length === 0 && (<Box sx={{ px: 2, py: 2, textAlign: 'center' }}><Typography sx={{ color: '#aaa', fontFamily: '"Noto Kufi Arabic",serif', fontSize: '0.82rem' }}>{t('search.noResults', { query: searchValue })}</Typography></Box>)}
                                    {searchValue.trim() && (<Box onMouseDown={e => { e.preventDefault(); executeSearch(searchValue); }} sx={{ px: 2, py: 1, borderTop: '1px solid #f0f0f0', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 1, bgcolor: 'rgba(8,101,168,0.02)', '&:hover': { bgcolor: 'rgba(8,101,168,0.08)' }, transition: 'background 0.15s' }}><SearchIcon sx={{ fontSize: 15, color: '#0865a8' }} /><Typography sx={{ fontFamily: '"Noto Kufi Arabic",serif', fontSize: '0.82rem', color: '#0865a8', fontWeight: 'bold', direction: isRTL ? 'rtl' : 'ltr' }}>{t('search.showAllResults', { query: searchValue })}</Typography></Box>)}
                                </Paper>
                            )}
                        </Box>
                    </ClickAwayListener>
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: { xs: 0.5, md: 1.5 }, flexShrink: 0 }}>
                        {!isMobile && (
                            <Stack direction="row" spacing={0.5} alignItems="center">
                                <div className="about-dropdown-wrapper">
                                    <Button color="inherit" endIcon={<KeyboardArrowDownIcon />} sx={{ px: 1.5, fontFamily: '"Noto Kufi Arabic",serif', color: '#000', fontSize: '0.9rem', '&:hover': { bgcolor: 'rgba(8,101,168,0.08)', color: '#0865a8' } }}>
                                        {t('nav.about')}
                                    </Button>
                                    <div className="about-dropdown-menu">
                                        {aboutLinks.map(item => {
                                            const isPdf = item.path.endsWith('.pdf');
                                            return (<Link key={item.ar} to={!isPdf ? item.path : '#'} onClick={isPdf ? e => { e.preventDefault(); window.open(item.path, '_blank'); } : undefined} className="dropdown-menu-item">{label(item)}</Link>);
                                        })}
                                    </div>
                                </div>
                                <Button color="inherit" component={Link} to="/news" sx={{ px: 1.5, fontFamily: '"Noto Kufi Arabic",serif', color: '#000', fontSize: '0.9rem', '&:hover': { bgcolor: 'rgba(8,101,168,0.08)', color: '#0865a8' } }}>
                                    {t('nav.news')}
                                </Button>
                                <Button color="inherit" component={Link} to="/library" sx={{ px: 1.5, fontFamily: '"Noto Kufi Arabic",serif', color: '#000', fontSize: '0.9rem', '&:hover': { bgcolor: 'rgba(8,101,168,0.08)', color: '#0865a8' } }}>
                                    {t('nav.library')}
                                </Button>
                                <div className="services-dropdown-wrapper">
                                    ...
                                    <div className="services-dropdown-menu">
                                        {serviceLinks.map((item, idx) => (<Link key={idx} to={item.path} className="dropdown-menu-item">{label(item)}</Link>))}
                                    </div>
                                </div>
                                {isAdmin && (
                        ...
                        <SignedIn>
                            <IconButton className="my-courses-nav-btn" component={Link} to="/my-courses" size="small" title={t('nav.myCourses')} sx={{ position: 'relative', transition: 'all 0.2s', color: '#0865a8', '&:hover': { backgroundColor: 'rgba(8,101,168,0.08)', transform: 'scale(1.05)' } }}>
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center' }}>
                            <SignedOut>
                                <SignInButton mode="modal">
                                    <Button variant="contained" size="small" sx={{ fontFamily: '"Noto Kufi Arabic",serif', fontSize: '0.8rem', bgcolor: '#0865a8', color: '#ffffff', px: 1, py: 0.5, borderRadius: 5, textTransform: 'none', fontWeight: 600, boxShadow: '0 4px 12px rgba(8,101,168,0.25)', minWidth: 'auto', '&:hover': { bgcolor: '#f57c00', boxShadow: '0 6px 18px rgba(245,124,0,0.35)' } }}>
                                        {t('nav.login')}
                                    </Button>
                                </SignInButton>
                            </SignedOut>
                ...
                <Box sx={{ width: 300, p: 2, bgcolor: 'white', height: '100%' }} dir={isRTL ? 'rtl' : 'ltr'}>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3, pb: 2, borderBottom: '2px solid #0865a8' }}>
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                            {/* ── Language toggle — Mobile drawer header ── */}
                    ...
                    <List>
                        ...
                        <ListItemButton onClick={() => setMobileCoursesOpen(!mobileCoursesOpen)} sx={{ bgcolor: mobileCoursesOpen ? 'rgba(8,101,168,0.08)' : 'transparent', borderRadius: 1, mb: 0.5, '&:hover': { bgcolor: 'rgba(8,101,168,0.12)' } }}>
                            <ListItemText primary={t('nav.courses')} sx={{ textAlign: isRTL ? 'right' : 'left', '& .MuiTypography-root': { fontFamily: '"Noto Kufi Arabic",serif', fontWeight: 'bold', color: '#0865a8' } }} />
                            {mobileCoursesOpen ? <ExpandMoreIcon sx={{ color: '#0865a8' }} /> : isRTL ? <ChevronLeftIcon sx={{ color: '#0865a8' }} /> : <ChevronRightIcon sx={{ color: '#0865a8' }} />}
                        </ListItemButton>
                        <Collapse in={mobileCoursesOpen} timeout="auto" unmountOnExit>
                            <List component="div" disablePadding sx={{ pr: isRTL ? 2 : 0, pl: isRTL ? 0 : 2 }}>
                                {loading ? (<ListItemButton><ListItemText primary={t('common.loading')} sx={{ '& .MuiTypography-root': { fontFamily: '"Noto Kufi Arabic",serif', fontSize: '0.9rem', color: '#666' } }} /></ListItemButton>)
                                    : error ? (<ListItemButton><ListItemText primary={t('common.error')} sx={{ '& .MuiTypography-root': { fontFamily: '"Noto Kufi Arabic",serif', fontSize: '0.9rem', color: '#f44336' } }} /></ListItemButton>)
                                        : mainCourses.map(course => (
                                            <React.Fragment key={course.id}>
                                                <ListItemButton onClick={() => { if (course.link) { navigate(course.link); toggleDrawer(false)(); } else if (course.sub) { setOpenSub(openSub === course.id ? null : course.id); } }} sx={{ bgcolor: 'rgba(0,0,0,0.02)', mb: 0.5, borderRadius: 1, '&:hover': { bgcolor: 'rgba(245,124,0,0.08)' } }}>
                                                    <ListItemText primary={course.title} sx={{ '& .MuiTypography-root': { fontFamily: '"Noto Kufi Arabic",serif', fontSize: '0.9rem' } }} />
                                                    {course.sub && (openSub === course.id ? <ExpandMoreIcon fontSize="small" sx={{ color: '#f57c00' }} /> : isRTL ? <ChevronLeftIcon fontSize="small" sx={{ color: '#0865a8' }} /> : <ChevronRightIcon fontSize="small" sx={{ color: '#0865a8' }} />)}
                                                </ListItemButton>
                                                {course.sub && (
                                                        ...
                                                        <List component="div" disablePadding sx={{ pr: isRTL ? 2 : 0, pl: isRTL ? 0 : 2 }}>
                                                            {course.sub.map(subItem => (
                                                                <React.Fragment key={subItem.id}>
                                                                    <ListItemButton onClick={() => subItem.topics && setOpenTopic(openTopic === subItem.id ? null : subItem.id)} sx={{ borderRight: isRTL ? '3px solid #0865a8' : 'none', borderLeft: isRTL ? 'none' : '3px solid #0865a8', '&:hover': { bgcolor: 'rgba(8,101,168,0.08)' } }}>
                                                                        <ListItemText primary={subItem.title} sx={{ '& .MuiTypography-root': { fontFamily: '"Noto Kufi Arabic",serif', fontSize: '0.85rem', color: '#0865a8', fontWeight: 'bold' } }} />
                                                                        {subItem.topics && (openTopic === subItem.id ? <ExpandMoreIcon fontSize="small" sx={{ color: '#f57c00' }} /> : isRTL ? <ChevronLeftIcon fontSize="small" sx={{ color: '#0865a8' }} /> : <ChevronRightIcon fontSize="small" sx={{ color: '#0865a8' }} />)}
                                                                    </ListItemButton>
                                                                    {subItem.topics && (
                                                                            ...
                                                                            <List component="div" disablePadding sx={{ pr: isRTL ? 2 : 0, pl: isRTL ? 0 : 2 }}>
                                                                                {subItem.topics.map(topic => (
                                                                                    <ListItemButton key={topic.id} component={Link} to={topic.link || '#'} onClick={toggleDrawer(false)} sx={{ '&:hover': { bgcolor: 'rgba(245,124,0,0.08)' } }}>
                                                                                        <ListItemText primary={topic.name} primaryTypographyProps={{ fontSize: '0.8rem', color: '#000', fontFamily: '"Noto Kufi Arabic",serif' }} />
                                                                                    </ListItemButton>
                                                                                ))}
                                                                            </List>
                                                                        </Collapse>
                        ...
                        <SignedIn>
                            <ListItemButton component={Link} to="/my-courses" onClick={toggleDrawer(false)} sx={{ borderRadius: 1, mb: 0.5, bgcolor: 'rgba(8,101,168,0.05)', '&:hover': { bgcolor: 'rgba(8,101,168,0.12)' } }}>
                                ...
                                <ListItemText primary={t('nav.myCourses')} primaryTypographyProps={{ fontFamily: '"Noto Kufi Arabic",serif', fontWeight: 'bold', color: '#0865a8' }} />
                            </ListItemButton>
                            <Divider sx={{ my: 2, bgcolor: '#0865a8', height: 2 }} />
                        </SignedIn>
                        ...
                        {isAdmin && (
                            <>
                                <ListItemButton component={Link} to="/Admin" onClick={toggleDrawer(false)} sx={{ borderRadius: 1, mb: 0.5, bgcolor: '#f57c00', border: '2px solid #f57c00', '&:hover': { bgcolor: '#e65100', borderColor: '#e65100' }, transition: 'all 0.2s' }}>
                                    <AdminPanelSettingsIcon sx={{ color: '#fff', mr: 1, fontSize: 20 }} />
                                    <ListItemText primary={t('nav.adminPanel')} primaryTypographyProps={{ fontFamily: '"Noto Kufi Arabic",serif', fontWeight: 'bold', color: '#fff' }} />
                                </ListItemButton>
                        ...
                        <ListItemButton onClick={() => setMobileAboutOpen(!mobileAboutOpen)} sx={{ bgcolor: mobileAboutOpen ? 'rgba(8,101,168,0.08)' : 'transparent', borderRadius: 1, mb: 0.5, '&:hover': { bgcolor: 'rgba(8,101,168,0.12)' } }}>
                            <ListItemText primary={t('nav.about')} sx={{ textAlign: isRTL ? 'right' : 'left', '& .MuiTypography-root': { fontFamily: '"Noto Kufi Arabic",serif', fontWeight: 'bold', color: '#0865a8' } }} />
                            {mobileAboutOpen ? <ExpandMoreIcon sx={{ color: '#0865a8' }} /> : isRTL ? <ChevronLeftIcon sx={{ color: '#0865a8' }} /> : <ChevronRightIcon sx={{ color: '#0865a8' }} />}
                        </ListItemButton>
                        <Collapse in={mobileAboutOpen} timeout="auto" unmountOnExit>
                            <List component="div" disablePadding sx={{ pr: isRTL ? 2 : 0, pl: isRTL ? 0 : 2 }}>
                                {aboutLinks.map(item => {
                                    const isPdf = item.path.endsWith('.pdf');
                                    return (<ListItemButton key={item.ar} component={isPdf ? 'a' : Link} to={!isPdf ? item.path : undefined} href={isPdf ? item.path : undefined} target={isPdf ? '_blank' : undefined} rel={isPdf ? 'noopener noreferrer' : undefined} onClick={!isPdf ? toggleDrawer(false) : undefined} sx={{ '&:hover': { bgcolor: 'rgba(245,124,0,0.08)' } }}><ListItemText primary={label(item)} primaryTypographyProps={{ fontFamily: '"Noto Kufi Arabic",serif', fontSize: '0.85rem' }} /></ListItemButton>);
                                })}
                            </List>
                        </Collapse>
                        <Divider sx={{ my: 2, bgcolor: '#0865a8', height: 2 }} />
                        ...
                        <ListItemButton onClick={() => setMobileServicesOpen(!mobileServicesOpen)} sx={{ bgcolor: mobileServicesOpen ? 'rgba(8,101,168,0.08)' : 'transparent', borderRadius: 1, mb: 0.5, '&:hover': { bgcolor: 'rgba(8,101,168,0.12)' } }}>
                            <ListItemText primary={t('nav.services')} sx={{ textAlign: isRTL ? 'right' : 'left', '& .MuiTypography-root': { fontFamily: '"Noto Kufi Arabic",serif', fontWeight: 'bold', color: '#0865a8' } }} />
                            {mobileServicesOpen ? <ExpandMoreIcon sx={{ color: '#0865a8' }} /> : isRTL ? <ChevronLeftIcon sx={{ color: '#0865a8' }} /> : <ChevronRightIcon sx={{ color: '#0865a8' }} />}
                        </ListItemButton>
                        <Collapse in={mobileServicesOpen} timeout="auto" unmountOnExit>
                            <List component="div" disablePadding sx={{ pr: isRTL ? 2 : 0, pl: isRTL ? 0 : 2 }}>
                                {serviceLinks.map((item, idx) => (<ListItemButton key={idx} component={Link} to={item.path} onClick={toggleDrawer(false)} sx={{ '&:hover': { bgcolor: 'rgba(245,124,0,0.08)' } }}><ListItemText primary={label(item)} primaryTypographyProps={{ fontFamily: '"Noto Kufi Arabic",serif', fontSize: '0.85rem' }} /></ListItemButton>))}
                            </List>
                        </Collapse>
                        <Divider sx={{ my: 2, bgcolor: '#0865a8', height: 2 }} />

                        <ListItemButton component={Link} to="/news" onClick={toggleDrawer(false)} sx={{ borderRadius: 1, mb: 0.5, '&:hover': { bgcolor: 'rgba(245,124,0,0.08)' } }}><ListItemText primary={t('nav.news')} primaryTypographyProps={{ fontFamily: '"Noto Kufi Arabic",serif', fontWeight: 'bold', color: '#0865a8' }} /></ListItemButton>
                        <ListItemButton component={Link} to="/library" onClick={toggleDrawer(false)} sx={{ borderRadius: 1, '&:hover': { bgcolor: 'rgba(245,124,0,0.08)' } }}><ListItemText primary={t('nav.library')} primaryTypographyProps={{ fontFamily: '"Noto Kufi Arabic",serif', fontWeight: 'bold', color: '#0865a8' }} /></ListItemButton>
                    </List>
                </Box>
            </Drawer>
