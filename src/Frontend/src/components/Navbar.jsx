...
const Navbar = () => {
    ...
    const transformCategories = (apiData) => {
        ...
        // Get the root category (الخطة التدريبية)
        ...
        return rootCategory.children.map(category => {
            ...
            if (category.children && category.children.length > 0) {
                ...
                if (!hasGrandchildren) {
                    transformed.sub = [{
                        ...
                        topics: category.children.map(child => ({
                            id: child.id,
                            name: child.title,
                            link: `/courses/${child.slug}`, // ✅ Changed from id to slug
                        }))
                    }];
                } else {
                        ...
                        if (subCategory.children && subCategory.children.length > 0) {
                            transformedSub.topics = subCategory.children.map(topic => {
                                const transformedTopic = {
                                    id: topic.id,
                                    name: topic.title,
                                    link: `/courses/${topic.slug}`, // ✅ Changed from id to slug
                                };
                                ...
                                if (topic.children && topic.children.length > 0) {
                                    transformedTopic.subTopics = topic.children.map(subTopic => {
                                        const transformedSubTopic = {
                                            id: subTopic.id,
                                            name: subTopic.title,
                                            link: `/courses/${subTopic.slug}`, // ✅ Changed from id to slug
                                        };
                                        ...
                                        if (subTopic.children && subTopic.children.length > 0) {
                                            transformedSubTopic.subSubTopics = subTopic.children.map(subSubTopic => ({
                                                ...
                                                link: `/courses/${subSubTopic.slug}`, // ✅ Changed from id to slug
                                            }));
                                        }
                ...
                // Category has no children, add direct link
    ...
    // Generate URL-friendly slug from Arabic title
        ...
        <>
            ...
            >
                <Toolbar sx={{ justifyContent: 'space-between', display: 'flex', px: { xs: 1, md: 4 } }}>
                    ...
                    {/* Desktop: Training Courses Button */}
                    {!isMobile && (
                        <Button
                            color="inherit"
                                    ...
                                    <Box>
                                        {activeCategory.sub.map((sub, index) => (
                                            ...
                                            >
                                                ...
                                                {/* Topics List - NOW SHOWING ALL TOPICS */}
                                                {sub.topics && (
                                                    <Box sx={{ pr: 4 }}>
...
//        // Map through children and transform them
//        return rootCategory.children.map(category => {
//            const transformed = {
//                id: category.id,
//                title: category.title,
...
//            if (category.children && category.children.length > 0) {
//                // Check if this is a simple list (like "برامج موجهة للماليين" with direct children)
//                const hasGrandchildren = category.children.some(child => child.children && child.children.length > 0);

//                if (!hasGrandchildren) {
//                    // Simple structure: category has direct topics (no sub-categories)
//                    transformed.sub = [{
//                        id: category.id + 1000, // Generate unique ID for the wrapper
//                        title: category.title,
//                        topics: category.children.map(child => ({
//                            id: child.id,
//                            name: child.title,
...
//                    }];
//                } else {
//                    // Complex structure: category has sub-categories
//                    transformed.sub = category.children.map(subCategory => {
//                        const transformedSub = {
//                            id: subCategory.id,
//                            title: subCategory.title,
...
//                                // Check if topic has sub-topics (like "المحور الأول" has children)
//                                if (topic.children && topic.children.length > 0) {
//                                    transformedTopic.subTopics = topic.children.map(subTopic => {
//                                        const transformedSubTopic = {
//                                            id: subTopic.id,
...
//                                        // Handle sub-sub-topics if they exist (like "برامج الجودة" has children)
//                                        if (subTopic.children && subTopic.children.length > 0) {
//                                            transformedSubTopic.subSubTopics = subTopic.children.map(subSubTopic => ({
//                                                id: subSubTopic.id,
//                                                name: subSubTopic.title,
...
//    // Generate URL-friendly slug from Arabic title
//    const generateSlug = (title) => {
//        // Remove special characters and extra spaces, keep Arabic and numbers
//        return title
//            .trim()
//            .replace(/\s+/g, '-')
//            .replace(/[^\w\u0600-\u06FF\u0660-\u0669-]/g, '')
...
//    const activeCategory = mainCourses.find(c => c.id === selectedCatId);
//    const activeSub = activeCategory?.sub?.find(s => s.id === selectedSubId);
...
//            {/* Desktop Courses Popover - CASCADING MENU */}
//            <Popover
//                open={Boolean(coursesAnchor)}
//                anchorEl={coursesAnchor}
//                onClose={handleClose}
...
//                disablePortal // helps with positioning & event bubbling
//                slotProps={{
//                    paper: {
//                        onMouseLeave: (e) => {
//                            // Check if we're still inside the popover or moving to a child
...
//            >
//                <Box sx={{
//                    display: 'flex', flexDirection: 'row', bgcolor: 'transparent',
//                    pointerEvents: 'auto',
//                }} dir="rtl">

//                    {/* Panel 1: Main Categories (Always visible) */}
//                    <Box sx={{
//                        width: '280px',
//                        bgcolor: '#fafafa',
//                        borderRadius: 2,
...
//                    {/* Panel 2: Sub-Categories (Shows when main category is hovered) */}
//                    {selectedCatId && activeCategory && activeCategory.sub && (
//                        <Box
//                            sx={{
//                                width: '300px',
//                                bgcolor: 'white',
...
//                                maxHeight: '420px',
//                                overflowY: 'auto',
//                                mr: -1,           // slight overlap / no gap – adjust if needed
//                                ml: 1,            // ← this pushes it to the right
...
//                                }
//                            }}
//                            onMouseEnter={() => setSelectedCatId(activeCategory.id)}
//                        >
//                            <Box sx={{ p: 2 }}>
//                                <Typography
//                                    sx={{
...
//                                                    fontSize: 13,
//                                                    fontFamily: '"Droid Arabic Kufi", serif',
//                                                    fontWeight: selectedSubId === sub.id ? 'bold' : '500',
//                                                    color: selectedSubId === sub.id ? '#0865a8' : '#333',
//                                                }}
//                                            />
//                                            {sub.topics && <ChevronLeftIcon sx={{ fontSize: 18, color: '#0865a8' }} />}
//                                        </ListItemButton>
//                                    ))}
//                                </List>
//                            </Box>
...
//                    {/* Panel 3: Topics (Shows when sub-category is hovered) */}
//                    {selectedSubId && activeSub && activeSub.topics && (
//                        <Box
//                            sx={{
//                                width: '320px',
//                                bgcolor: 'white',
...
//                                }
//                            }}
//                            onMouseEnter={() => setSelectedSubId(activeSub.id)}
//                        >
//                            <Box sx={{ p: 2 }}>
//                                <Typography
//                                    sx={{
