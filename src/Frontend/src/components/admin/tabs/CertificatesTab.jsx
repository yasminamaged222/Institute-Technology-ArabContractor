...
export function CertificatesTab({ rows, loading, certificates, certUploading, certDeleting, error, setError, search, setSearch, statusFilter, setStatusFilter, currentPage, setCurrentPage, attendance, onOpenModal, onViewCert, onDeleteCert, setExporting, setExportError }) {
    ...
    // summary numbers
    const uploaded = rows.filter(r => !!(certificates[r.certKey] ?? (r.altKey ? certificates[r.altKey] : undefined))).length;
    const withUrl = rows.filter(r => { const c = certificates[r.certKey] ?? (r.altKey ? certificates[r.altKey] : undefined); return c?.url && c.url !== 'uploaded'; }).length;
    const attendedTotal = rows.filter(r => !!attendance[String(r.enrollmentId)]).length;
    const pendingUp = rows.filter(r => !(certificates[r.certKey] ?? (r.altKey ? certificates[r.altKey] : undefined)) && !!attendance[String(r.enrollmentId)]).length;
    ...
    return (
        <>
            <div className="adm-section-hdr">
                <div><div className="adm-section-tag">إدارة الشهادات</div><div className="adm-section-title"><span>الشهادات</span> الإلكترونية</div></div>
            </div>
            <div className="adm-toolbar">
                <div className="adm-search" style={{ minWidth: 210 }}>
                    <input type="text" placeholder="ابحث باسم المستخدم أو الدورة..." value={search} onChange={e => setSearch(e.target.value)} />
                </div>
                ...
                <div style={{ display: 'flex', gap: 5, marginRight: 'auto', flexWrap: 'wrap' }}>
                    {STATUS_FILTERS.map(f => (
                        <button key={f.id} style={{ padding: '5px 12px', borderRadius: 2, border: '1.5px solid', fontFamily: T.font, fontSize: '.7rem', fontWeight: 700, cursor: 'pointer', transition: 'all .14s', background: statusFilter === f.id ? (f.id === 'uploaded' ? '#f0fdf4' : 'rgba(8,101,168,0.08)') : T.gray100, borderColor: statusFilter === f.id ? (f.id === 'uploaded' ? '#86efac' : 'rgba(8,101,168,0.3)') : T.gray300, color: statusFilter === f.id ? (f.id === 'uploaded' ? '#15803d' : T.blue) : T.gray500 }} onClick={() => setStatusFilter(f.id)}>{f.icon} {f.label}</button>
                    ))}
                </div>
            ...
            {!loading && (
                <div className="adm-cert-sum">
                    ...
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', flex: 1 }}>
                        ...
                        ].map(b => <span key={b.label} style={{ display: 'inline-flex', alignItems: 'center', padding: '3px 11px', borderRadius: 2, background: b.bg, border: `1.5px solid ${b.border}`, color: b.color, fontSize: '.7rem', fontWeight: 700 }}>{b.label}</span>)}
                    </div>
                        ...
                        : (<>
                            <div className="adm-cert-grid">
                                {paginated.map(row => {
                                    ...
                                    const iconCls = cert ? 'has' : !isAtt ? 'grey' : '';
                                    return (
                                        <div className={`adm-cert-card ${cardCls}`} key={ck} style={{ opacity: !isAtt && !cert ? 0.75 : 1 }}>
                                            <div className="adm-cert-card-top">
                                                <div className={`adm-cert-icon${iconCls ? ` ${iconCls}` : ''}`}>{cert ? '📜' : isAtt ? '📄' : '🚫'}</div>
                                                <div className="adm-cert-info">
                                                    ...
                                                    <div className="adm-cert-badges">
                                                        <span style={{ fontSize: '.6rem', fontWeight: 700, padding: '2px 8px', borderRadius: 2, background: isAtt ? '#f0fdf4' : T.gray100, color: isAtt ? '#15803d' : T.gray500, border: `1px solid ${isAtt ? '#86efac' : T.gray300}` }}>{isAtt ? '✅ حضر' : '❌ غائب'}</span>
                                                        {cert?.certId != null && <span style={{ fontSize: '.6rem', fontWeight: 700, padding: '2px 8px', borderRadius: 2, background: '#f0fdf4', color: '#15803d', border: '1px solid #86efac' }}>📜 {cert.name && cert.name !== 'uploaded' ? (cert.name.length > 20 ? cert.name.slice(0, 20) + '…' : cert.name) : 'مرفوعة'}</span>}
                                                        {!cert && isAtt && <span style={{ fontSize: '.6rem', color: T.orange, padding: '2px 8px', borderRadius: 2, background: 'rgba(245,124,0,0.06)', border: '1px solid rgba(245,124,0,0.2)' }}>لم تُرفع بعد</span>}
                                                        {!cert && !isAtt && <span style={{ fontSize: '.6rem', color: T.gray500, padding: '2px 8px', borderRadius: 2, background: T.gray100, border: `1px solid ${T.gray300}` }}>يجب تسجيل الحضور أولاً</span>}
                                            ...
                                            <div className="adm-cert-actions">
                                                {cert ? (
                                                    <>
                                                        <button className="adm-cert-btn dl" onClick={() => onViewCert(cert.certId, cert.url, cert.rawUrl, cert.name, row.userId, row.planworkId)}>👁 عرض</button>
                                                        <button className="adm-cert-btn up" disabled={uploading} onClick={() => onOpenModal({ enrollmentId: row.enrollmentId, userId: row.userId, planworkId: row.planworkId, certKey: ck, userName: `${row.user.firstName || row.user.username} ${row.user.lastName}`, courseTitle: row.course.title })}>{uploading ? '⏳' : '🔄 تحديث'}</button>
                                                        <button className="adm-cert-btn rm" disabled={deleting} onClick={() => onDeleteCert(ck, row.altKey)}>{deleting ? '⏳' : '🗑'}</button>
                                                    </>
                                                ) : isAtt ? (
                                                    <button className="adm-cert-btn up full" disabled={uploading} onClick={() => onOpenModal({ enrollmentId: row.enrollmentId, userId: row.userId, planworkId: row.planworkId, certKey: ck, userName: `${row.user.firstName || row.user.username} ${row.user.lastName}`, courseTitle: row.course.title })}>{uploading ? '⏳ جاري الرفع...' : '⬆ رفع شهادة'}</button>
                                                ) : (
                                                    <span style={{ fontSize: '.62rem', color: T.gray300, width: '100%', textAlign: 'center' }}>سجّل الحضور أولاً لرفع الشهادة</span>
                                                )}
                                            </div>
                                        </div>
                                    );
                            ...
                            <Pagination currentPage={currentPage} totalItems={rows.length} itemsPerPage={ITEMS_PER_PAGE} onPageChange={setCurrentPage} accentColor={T.orange} />
                        </>)}
            </div>
        </>
    );
